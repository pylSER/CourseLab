    ...
    body:jsondata})).then( (response)=> {
      response.json().then(function(data) {
            //  console.log(data);
                ...
                if(data.username==that.state.loginName && data.type=='student'){
                  //ok to show data
                  that.setState({realName:data.name});
                  that.setState({email:data.email});
                  that.setState({gitID:data.gitId});
                  ...
                  if(data.number){
                      that.setState({usernumber:data.number});
                  }

                  if(data.gitUsername){
                      that.setState({gitUserName:data.gitUsername});
                  }
...
<EmailItem title='电子邮箱' content={this.state.email} />
  ...
  <InfoItem title='git 用户名' content={this.state.gitUserName} />
